<template>
  <WMSPageLayout>
    <WMSSearchForm :model="query" @submit="handleQuery" @reset="resetQuery">
      <template #form-items>
        <a-col :span="6">
          <a-form-item field="code" label="客户编码">
            <a-input v-model="query.code" placeholder="请输入客户编码" />
          </a-form-item>
        </a-col>
        <a-col :span="6">
          <a-form-item field="name" label="客户名称">
            <a-input v-model="query.name" placeholder="请输入客户名称" />
          </a-form-item>
        </a-col>
        <a-col :span="6">
        ...
        <a-row :gutter="16">
          <a-col :span="12">
            <a-form-item field="code" label="客户编码" required>
              <a-input v-model="formData.code" placeholder="请输入客户编码" />
            </a-form-item>
          </a-col>
          <a-col :span="12">
...
import { getCustomerList, addCustomer, updateCustomer, deleteCustomer } from '@/apis/wms/customerService';
import type { Customer, ItemQuery } from '@/types/wms';
...
const formRules = {
  code: [
    ...
    { minLength: 1, maxLength: 32, message: '客户编码长度应在1-32个字符之间' }
  ],
  name: [
    { required: true, message: '请输入客户名称' },
    { minLength: 1, maxLength: 100, message: '客户名称长度应在1-100个字符之间' }
  ],
  status: [
...
const handleSelect = (record: Customer, checked: boolean) => {
  if (checked) {
...
const handleSelectAll = (checked: boolean, data: Customer[]) => {
  if (checked) {
